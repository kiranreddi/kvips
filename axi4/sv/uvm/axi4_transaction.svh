...
class axi4_item #(
  ...
  constraint c_no_4kb_cross {
    if (!allow_4kb_cross) {
      if (burst != AXI4_BURST_FIXED) !axi4_crosses_4kb(longint unsigned'(addr), int'(size), int'(len));
    }
  }

  // Enforce WRAP start-address alignment to the wrap container.
  constraint c_wrap_align {
    if (!allow_wrap_misalign && (burst == AXI4_BURST_WRAP)) axi4_wrap_addr_aligned(longint unsigned'(addr), int'(size), int'(len));
  }
