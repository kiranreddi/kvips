...
if [[ "${REUSE_BUILD}" != "1" || ! -x "${BIN}" ]]; then
  ${VERILATOR_BIN} -sv --language 1800-2017 -Wno-fatal -Wno-PKGNODECL -Wno-UNDRIVEN -Wno-TIMESCALEMOD -Wno-SYNCASYNCNET -Wno-MISINDENT -Wno-UNSUPPORTED -Wno-WIDTHTRUNC -Wno-WIDTHEXPAND --timing --binary -j "${JOBS}" \
...
[[ "$HAVE_TESTNAME" -eq 0 ]] && EXTRA_ARGS+=("+UVM_TESTNAME=axi4_b2b_smoke_test")
