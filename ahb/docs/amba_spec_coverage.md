...
| Transfer alignment and bus-width legality | Covered on master stimulus | `ahb_cfg.legal_transfer()` and boundary test; negative illegal-traffic test is not yet included |
...
| `HPROT` transport | Covered as transaction metadata | No downstream protection/firewall policy is enforced |
| AHB5 `HNONSEC` transport | Covered | Interface, driver, monitor, transaction metadata; BUSY test drives non-secure traffic |
| `HMASTLOCK/HMASTER` fabric semantics | Partial | Signals exist in the superset interface; no multi-master scheduler/arbitration |
...
1. Add a multi-master arbiter/interconnect example with `HMASTER`, lock, and
   `HSPLITx` routing.
2. Add negative tests that intentionally violate alignment, burst length, and
   1KB rules and prove the checker catches them.
3. Add security-policy callbacks for `HNONSEC`/`HPROT` rather than only
   transporting those fields.
