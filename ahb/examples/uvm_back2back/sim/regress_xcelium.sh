...
run_log_has_issue() {
  local log="$1"
  grep -Eq 'UVM_ERROR[[:space:]]*:[[:space:]]*[1-9]|UVM_FATAL[[:space:]]*:[[:space:]]*[1-9]|Number of caught UVM_(ERROR|FATAL) reports[[:space:]]*:[[:space:]]*[1-9]|Errors: [1-9]' "${log}"
}
...
while IFS= read -r test_name || [[ -n "${test_name}" ]]; do
  ...
  make -s -C "${ROOT}/ahb/examples" xcelium TEST="${test_name}" \
    ...
    PLUSARGS="${plusargs}"
  if run_log_has_issue "${ROOT}/ahb/examples/uvm_back2back/sim/out/xcelium/run.log"; then
    echo "FAIL: ${test_name} (Xcelium reported a nonzero UVM/simulator error count)" >&2
    exit 1
  fi
done < "${TESTS_FILE}"
