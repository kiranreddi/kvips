...
OUT="${ROOT}/ahb/examples/uvm_dut/sim/out/xcelium"
...
command -v xrun >/dev/null 2>&1 || { echo "ERROR: Xcelium is unavailable; load xcelium/25.03.007." >&2; exit 127; }
make_abs_filelist "${FILELIST}" "${ABS_FILELIST}"
cd "${OUT}"
if [[ "${KVIPS_REUSE_BUILD:-0}" != "1" ]]; then
  rm -rf xcelium.d INCA_libs *.shm *.history
fi
args=("$@")
...
xrun -64bit -sv -timescale 1ns/1ps -f "${ABS_FILELIST}" -uvm -access +rwc "${args[@]}" -l xrun.log
